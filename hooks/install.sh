#!/bin/bash
# tinker — one-command hook installer for Claude Code
# Installs: SessionStart hook (auto-load rules) + UserPromptSubmit hook (mode tracking)
# Usage: bash hooks/install.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/JuliusBrussee/mad-tinker/main/hooks/install.sh)
...
if ! command -v node >/dev/null 2>&1; then
  echo "ERROR: 'node' is required to install the tinker hooks (used to merge"
...
REPO_URL="https://raw.githubusercontent.com/JuliusBrussee/mad-tinker/main/hooks"

HOOK_FILES=("package.json" "tinker-config.js" "tinker-activate.js" "tinker-mode-tracker.js" "tinker-statusline.sh")
...
if [ "$FORCE" -eq 0 ]; then
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ -f "$SETTINGS" ]; then
    if TINKER_SETTINGS="$SETTINGS" node -e "
      const fs = require('fs');
      const settings = JSON.parse(fs.readFileSync(process.env.TINKER_SETTINGS, 'utf8'));
      const hasMad TinkerHook = (event) =>
        Array.isArray(settings.hooks?.[event]) &&
        settings.hooks[event].some(e =>
          e.hooks && e.hooks.some(h => h.command && h.command.includes('tinker'))
        );
      process.exit(
        hasMad TinkerHook('SessionStart') &&
        hasMad TinkerHook('UserPromptSubmit') &&
        !!settings.statusLine
          ? 0
          : 1
      );
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ "$HOOKS_WIRED" -eq 1 ] && [ "$HAS_STATUSLINE" -eq 1 ]; then
    ALREADY_INSTALLED=1
    echo "Mad Tinker hooks already installed in $HOOKS_DIR"
...
if [ "$FORCE" -eq 1 ] && [ -f "$HOOKS_DIR/tinker-activate.js" ]; then
  echo "Reinstalling tinker hooks (--force)..."
else
  echo "Installing tinker hooks..."
fi
...
chmod +x "$HOOKS_DIR/tinker-statusline.sh"
...
TINKER_SETTINGS="$SETTINGS" TINKER_HOOKS_DIR="$HOOKS_DIR" node -e "
  const fs = require('fs');
  const settingsPath = process.env.TINKER_SETTINGS;
  const hooksDir = process.env.TINKER_HOOKS_DIR;
  const managedStatusLinePath = hooksDir + '/tinker-statusline.sh';
  const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
  if (!settings.hooks) settings.hooks = {};

  // SessionStart — auto-load tinker rules
  ...
  if (!hasStart) {
    settings.hooks.SessionStart.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/tinker-activate.js\"',
        timeout: 5,
        statusMessage: 'Loading tinker mode...'
      }]
    });
  }

  // UserPromptSubmit — track mode changes when user types /tinker commands
  ...
  if (!hasPrompt) {
    settings.hooks.UserPromptSubmit.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/tinker-mode-tracker.js\"',
        timeout: 5,
        statusMessage: 'Tracking tinker mode...'
      }]
    });
  }

  // Statusline — wire tinker badge (report if skipped)
  if (!settings.statusLine) {
    ...
    if (cmd.includes(managedStatusLinePath)) {
      ...
      console.log('  NOTE: Existing statusline detected — tinker badge NOT added.');
...
echo "  - SessionStart hook: auto-loads tinker rules every session"
echo "  - Mode tracker hook: updates statusline badge when you switch modes"
echo "    (/tinker lite, /tinker ultra, /tinker-commit, etc.)"
echo "  - Statusline badge: shows [TINKER] or [TINKER:ULTRA] etc."
