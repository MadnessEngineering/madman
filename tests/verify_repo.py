#!/usr/bin/env python3
"""Local verification runner for tinker install surfaces."""
...
def verify_synced_files() -> None:
    ...
    skill_source = ROOT / "skills/mad-tinker/SKILL.md"
    rule_source = ROOT / "rules/mad-tinker-activate.md"

    skill_copies = [
        ROOT / "mad-tinker/SKILL.md",
        ROOT / "plugins/mad-tinker/skills/mad-tinker/SKILL.md",
        ROOT / ".cursor/skills/mad-tinker/SKILL.md",
        ROOT / ".windsurf/skills/mad-tinker/SKILL.md",
    ]
    for copy in skill_copies:
        ensure(copy.read_text() == skill_source.read_text(), f"Skill copy mismatch: {copy}")

    rule_copies = [
        ROOT / ".clinerules/mad-tinker.md",
        ROOT / ".github/copilot-instructions.md",
    ]
    for copy in rule_copies:
        ensure(copy.read_text() == rule_source.read_text(), f"Rule copy mismatch: {copy}")

    with zipfile.ZipFile(ROOT / "mad-tinker.skill") as archive:
        ensure("mad-tinker/SKILL.md" in archive.namelist(), "mad-tinker.skill missing mad-tinker/SKILL.md")
        ensure(
            archive.read("mad-tinker/SKILL.md").decode("utf-8") == skill_source.read_text(),
            "mad-tinker.skill payload mismatch",
        )

    print("Synced copies and mad-tinker.skill zip OK")
...
def verify_manifests_and_syntax() -> None:
    ...
    manifest_paths = [
        ...
        ROOT / "plugins/mad-tinker/.codex-plugin/plugin.json",
    ]
    for path in manifest_paths:
        read_json(path)

    run(["node", "--check", "hooks/tinker-config.js"])
    run(["node", "--check", "hooks/tinker-activate.js"])
    run(["node", "--check", "hooks/tinker-mode-tracker.js"])
    run(["bash", "-n", "hooks/install.sh"])
    run(["bash", "-n", "hooks/uninstall.sh"])
    run(["bash", "-n", "hooks/tinker-statusline.sh"])

    # Ensure install/uninstall scripts include tinker-config.js
    install_sh = (ROOT / "hooks/install.sh").read_text()
    uninstall_sh = (ROOT / "hooks/uninstall.sh").read_text()
    ensure("tinker-config.js" in install_sh, "install.sh missing tinker-config.js")
    ensure("tinker-config.js" in uninstall_sh, "uninstall.sh missing tinker-config.js")
...
def verify_powershell_static() -> None:
    ...
    statusline_text = (ROOT / "hooks/tinker-statusline.ps1").read_text()

    ensure("tinker-config.js" in install_text, "install.ps1 missing tinker-config.js")
    ensure("tinker-config.js" in uninstall_text, "uninstall.ps1 missing tinker-config.js")
    ensure("tinker-statusline.ps1" in install_text, "install.ps1 missing statusline.ps1")
    ensure("tinker-statusline.ps1" in uninstall_text, "uninstall.ps1 missing statusline.ps1")
    ...
    ensure("[TINKER" in statusline_text, "tinker-statusline.ps1 missing badge output")

    print("Windows install path statically wired")


def load_compress_modules():
    sys.path.insert(0, str(ROOT / "tinker-compress"))
...
def verify_compress_fixtures() -> None:
    ...
    fixtures = sorted((ROOT / "tests/tinker-compress").glob("*.original.md"))
    ensure(fixtures, "No tinker-compress fixtures found")
    ...
    print(f"Validated {len(fixtures)} tinker-compress fixture pairs")
...
def verify_hook_install_flow() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="tinker-verify-") as temp_root:
        ...
        activate = run(
            ["node", "hooks/tinker-activate.js"],
            env={"HOME": str(home)},
        )
        ensure("TINKER MODE ACTIVE." in activate.stdout, "activation output missing tinker banner")
        ensure("STATUSLINE SETUP NEEDED" not in activate.stdout, "activation should stay quiet when custom statusline exists")
        ensure((claude_dir / ".tinker-active").read_text() == "full", "activation flag should default to full")

        # Test configurable default mode via TINKER_DEFAULT_MODE env var
        activate_custom = run(
            ["node", "hooks/tinker-activate.js"],
            env={"HOME": str(home), "TINKER_DEFAULT_MODE": "ultra"},
        )
        ensure("TINKER MODE ACTIVE." in activate_custom.stdout, "activation with custom default missing banner")
        ensure((claude_dir / ".tinker-active").read_text() == "ultra", "TINKER_DEFAULT_MODE=ultra should set flag to ultra")
        # Test "off" mode — activation skipped, flag removed
        activate_off = run(
            ["node", "hooks/tinker-activate.js"],
            env={"HOME": str(home), "TINKER_DEFAULT_MODE": "off"},
        )
        ensure("TINKER MODE ACTIVE." not in activate_off.stdout, "off mode should not emit tinker banner")
        ensure(not (claude_dir / ".tinker-active").exists(), "off mode should remove flag file")

        # Test mode tracker with /tinker when default is off — should NOT write flag
        subprocess.run(
            ["node", "hooks/tinker-mode-tracker.js"],
            cwd=ROOT,
            env={**os.environ, "HOME": str(home), "TINKER_DEFAULT_MODE": "off"},
            text=True,
            input='{"prompt":"/tinker"}',
            capture_output=True,
            check=True,
        )
        ensure(not (claude_dir / ".tinker-active").exists(), "/tinker with off default should not write flag")

        # Reset back to full for subsequent tests
        (claude_dir / ".tinker-active").write_text("full")
        ...
        ultra_prompt = subprocess.run(
            ...
            text=True,
            input='{"prompt":"/tinker ultra"}',
            capture_output=True,
            check=True,
        )
        ensure(ultra_prompt.stdout == "", "mode tracker should stay silent")
        ensure((claude_dir / ".tinker-active").read_text() == "ultra", "mode tracker did not record ultra")
        ...
        ensure(not (claude_dir / ".tinker-active").exists(), "normal mode should remove flag file")

        (claude_dir / ".tinker-active").write_text("tinker-ultra")
        statusline = run(
            ["bash", "hooks/tinker-statusline.sh"],
            env={"HOME": str(home)},
        )
        ensure("[TINKER:TINKER-ULTRA]" in statusline.stdout, "statusline badge output mismatch")
        ...
        ensure(settings_after == existing_settings, "uninstall.sh did not restore non-tinker settings")
        ensure(not (claude_dir / ".tinker-active").exists(), "uninstall.sh should remove flag file")

    with tempfile.TemporaryDirectory(prefix="tinker-verify-fresh-") as temp_root:
        home = Path(temp_root) / "home"
        run(["bash", "hooks/install.sh"], env={"HOME": str(home)})
        claude_dir = home / ".claude"
        settings = read_json(claude_dir / "settings.json")
        ensure("statusLine" in settings, "fresh install should configure statusline")
        activate = run(["node", "hooks/tinker-activate.js"], env={"HOME": str(home)})
