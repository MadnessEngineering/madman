#!/usr/bin/env python3
"""Benchmark tinker vs normal Claude output token counts."""
...
SKILL_PATH = REPO_DIR / "skills" / "tinker" / "SKILL.md"
...
def run_benchmarks(client, model, prompts, tinker_system, trials):
    results = []
    total = len(prompts)
    ...
    for i, prompt_entry in enumerate(prompts, 1):
        ...
        for mode, system in [("normal", NORMAL_SYSTEM), ("tinker", tinker_system)]:
...
def compute_stats(results):
    ...
    for entry in results:
        ...
        tinker_medians = statistics.median(
            [t["output_tokens"] for t in entry["tinker"]]
        )
        savings = 1 - (tinker_medians / normal_medians) if normal_medians > 0 else 0
        ...
        rows.append(
            {
                ...
                "tinker_median": int(tinker_medians),
    ...
    avg_tinker = round(statistics.mean([r["tinker_median"] for r in rows]))
...
def format_table(rows, summary):
    lines = [
        "| Task | Normal (tokens) | Mad Tinker (tokens) | Saved |",
        "|------|---------------:|----------------:|------:|",
    ]
    for r in rows:
        label = format_prompt_label(r["id"])
        lines.append(
            f"| {label} | {r['normal_median']} | {r['tinker_median']} | {r['savings_pct']}% |"
        )
    lines.append(
        f"| **Average** | **{summary['avg_normal']}** | **{summary['avg_tinker']}** | **{summary['avg_savings']}%** |"
    )
    lines.append("")
    lines.append(
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark tinker vs normal Claude")
    ...
    tinker_system = load_tinker_system()
    ...
    results = run_benchmarks(client, args.model, prompts, tinker_system, args.trials)
