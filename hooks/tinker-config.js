#!/usr/bin/env node
// tinker — shared configuration resolver
//
// Resolution order for default mode:
//   1. TINKER_DEFAULT_MODE environment variable
//   2. Config file defaultMode field:
//      - $XDG_CONFIG_HOME/tinker/config.json (any platform, if set)
//      - ~/.config/tinker/config.json (macOS / Linux fallback)
//      - %APPDATA%\tinker\config.json (Windows fallback)
//   3. 'full'
...
function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'tinker');
  }
  ...
  return path.join(os.homedir(), '.config', 'tinker');
}
...
function getDefaultMode() {
  ...
  const envMode = process.env.TINKER_DEFAULT_MODE;
...
// flag path (~/.claude/.tinker-active) with a symlink to clobber other files.
//
...
function safeWriteFlag(flagPath, content) {
  try {
    ...
    const tempPath = path.join(flagDir, `.tinker-active.${process.pid}.${Date.now()}`);
...
//
// MAX_FLAG_BYTES is a hard cap. The longest legitimate value is "tinker-ultra"
