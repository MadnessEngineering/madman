...
$Flag = Join-Path $ClaudeDir ".tinker-active"
...
$Valid = @('off','lite','full','ultra','commit','review','compress','tinker','tinker-lite','tinker-ultra','tinker-compressed')
if (-not ($Valid -contains $Mode)) { exit 0 }

$Esc = [char]27
if ($Mode.StartsWith("tinker")) {
    if ($Mode -eq "tinker") {
        [Console]::Write("${Esc}[38;5;34m[TINKER]${Esc}[0m")
    } else {
        $Suffix = ($Mode -replace 'tinker-', '').ToUpperInvariant()
        [Console]::Write("${Esc}[38;5;34m[TINKER:$Suffix]${Esc}[0m")
    }
} elseif ([string]::IsNullOrEmpty($Mode) -or $Mode -eq "full") {
    [Console]::Write("${Esc}[38;5;172m[TINKER]${Esc}[0m")
} else {
    $Suffix = $Mode.ToUpperInvariant()
    [Console]::Write("${Esc}[38;5;172m[TINKER:$Suffix]${Esc}[0m")
}
