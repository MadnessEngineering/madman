# tinker — one-command hook installer for Claude Code (Windows PowerShell)
...
#        irm https://raw.githubusercontent.com/JuliusBrussee/mad-tinker/main/hooks/install.ps1 | iex
...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
    Write-Host "ERROR: 'node' is required to install the tinker hooks (used to merge" -ForegroundColor Red
...
$RepoUrl = "https://raw.githubusercontent.com/JuliusBrussee/mad-tinker/main/hooks"

$HookFiles = @("package.json", "tinker-config.js", "tinker-activate.js", "tinker-mode-tracker.js", "tinker-statusline.sh", "tinker-statusline.ps1")
...
if (-not $Force) {
    ...
    if ($AllFilesPresent -and (Test-Path $Settings)) {
        try {
                ...
                foreach ($entry in $entries) {
                    if ($entry.hooks) {
                        foreach ($hookDef in $entry.hooks) {
                            if ($hookDef.command -and $hookDef.command.Contains("tinker")) {
                                return $true
                            }
                        }
                    }
            ...
            $HooksWired = (& $hasMad TinkerHook "SessionStart") -and (& $hasMad TinkerHook "UserPromptSubmit")
    ...
    if ($AllFilesPresent -and $HooksWired -and $HasStatusLine) {
        Write-Host "Mad Tinker hooks already installed in $HooksDir"
...
if ($Force -and (Test-Path (Join-Path $HooksDir "tinker-activate.js"))) {
    Write-Host "Reinstalling tinker hooks (-Force)..."
} else {
    Write-Host "Installing tinker hooks..."
}
...
$env:TINKER_SETTINGS = $Settings -replace '\\', '/'
$env:TINKER_HOOKS_DIR = $HooksDir -replace '\\', '/'

$nodeScript = @'
const fs = require('fs');
const settingsPath = process.env.TINKER_SETTINGS;
const hooksDir = process.env.TINKER_HOOKS_DIR;
const managedStatusLinePath = hooksDir + '/tinker-statusline.ps1';
...
Write-Host "  - SessionStart hook: auto-loads tinker rules every session"
Write-Host "  - Mode tracker hook: updates statusline badge when you switch modes"
Write-Host "    (/tinker lite, /tinker ultra, /tinker-commit, etc.)"
Write-Host "  - Statusline badge: shows [TINKER] or [TINKER:ULTRA] etc."
